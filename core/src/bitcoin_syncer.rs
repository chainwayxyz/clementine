...
/// Fetches the next block from Bitcoin, if exists. Will also fetch previous
...
impl Task for BitcoinSyncerTask {
    ...
    async fn run_once(&mut self) -> Result<Self::Output, BridgeError> {
        let new_blocks = match fetch_next_block(&self.db, &self.rpc, self.current_height).await? {
...
mod tests {
    ...
    async fn set_initial_block_info_if_not_exists() {
        ...
        let hash = rpc
            .client
            .get_block_hash(config.protocol_paramset.start_height.into())
            .await
            .unwrap();
        let block = rpc.client.get_block(&hash).await.unwrap();
    ...
    async fn fetch_new_blocks_forward() {
        ...
        let new_blocks = super::fetch_next_block(&db, &rpc, height).await.unwrap();
        ...
        let new_blocks = super::fetch_next_block(&db, &rpc, height)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn fetch_new_blocks_backwards() {
        ...
        let new_blocks = super::fetch_next_block(&db, &rpc, new_height - 1)
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(super::fetch_next_block(&db, &rpc, new_height - 1)
            .await
            .is_err());
    }
    #[ignore]
