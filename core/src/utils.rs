...
pub fn from_hex_to_bytes(input: &str) -> ([u8; 256], usize) {
...
pub fn from_hex_to_tx(input: &str) -> Transaction {
    ...
    index += 2;
    let mut inputs = [TxInput::empty(); 1];
    let input_count = from_hex_to_u8(hex_input_count);
    for i in 0..input_count {
        let hex_tx_id = &input[index..index + 64];
        let tx_id: [u8; 32] = from_hex_to_bytes(hex_tx_id).0[..32].try_into().unwrap();
        index += 64;
        ...
        index += 2;
        let script_sig = from_hex_to_bytes(&input[index..index + (script_sig_size as usize) * 2]).0;
    ...
    index += 2;
    let mut outputs = [TxOutput::empty(); 25];
