...
pub fn create_slash_or_take_tx(
    ...
    let relative_timelock_script =
        script_builder::generate_relative_timelock_script(operator_xonly_pk, OPERATOR_TAKES_AFTER); // TODO: Change this 200 to a config constant
...
pub fn create_operator_takes_tx(
    ...
    let operator_address = Address::p2tr(&utils::SECP, *operator_xonly_pk, None, network);
    let ins = create_tx_ins(vec![bridge_fund_outpoint, slash_or_take_utxo.outpoint]);
...
//     use crate::{config::BridgeConfig, TransactionBuilder};
//     use std::str::FromStr;
...
//         let tx_builder = new(verifier_pks, config.network);
