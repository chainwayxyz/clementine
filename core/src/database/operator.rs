...
impl Database {
    ...
    pub async fn get_signatures_for_unused_and_signed_kickoff_connector(
        &self,
        tx: Option<DatabaseTransaction<'_, '_>>,
        deposit_id: u32,
    ) -> Result<Option<(u32, u32, Vec<TaggedSignature>)>, BridgeError> {
        let query = sqlx::query_as::<_, (i32, i32, SignaturesDB)>(
        ...
        match result {
            Some((round_idx, kickoff_connector_idx, signatures)) => Ok(Some((
