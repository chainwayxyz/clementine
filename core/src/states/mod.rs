...
impl<T: Owner + std::fmt::Debug + 'static> StateManager<T> {
    ...
    fn match_blocks<M>(
        ...
        Vec<(InitializedStateMachine<M>, Vec<M::StateEvent>)>,
    )
    where
        ...
        for<'evt> M::Event<'evt>: Send + Sync,
    {
        ...
        for mut machine in std::mem::take(machines).into_iter() {
            let events = machine.match_block(&cache);
            if events.is_empty() {
                unchanged_machines.push(machine);
            } else {
                processing_futures.push((machine, events));
            }
        }
    ...
    pub async fn process_block_parallel(&mut self, block_height: u32) -> Result<(), eyre::Report> {
        ...
        let (mut final_kickoff_machines, mut kickoff_futures) =
            Self::match_blocks(&mut self.kickoff_machines, self.context.cache.clone());
        let (mut final_round_machines, mut round_futures) =
            Self::match_blocks(&mut self.round_machines, self.context.cache.clone());
        ...
        while !kickoff_futures.is_empty() || !round_futures.is_empty() {
            // Execute all futures in series
            let ctx = &mut self.context;

            let (mut updated_kickoffs, mut updated_rounds) = {
                let mut updated_kickoffs = Vec::new();
                let mut updated_rounds = Vec::new();

                for (mut machine, events) in kickoff_futures.into_iter() {
                    for event in events {
                        machine.handle_with_context(&event, ctx).await;
                    }
                    updated_kickoffs.push(machine);
                }
                for (mut machine, events) in round_futures.into_iter() {
                    for event in events {
                        machine.handle_with_context(&event, ctx).await;
                    }
                    updated_rounds.push(machine);
                }

                (updated_kickoffs, updated_rounds)
            };
            ...
            updated_rounds.extend(std::mem::take(&mut ctx.new_round_machines));
            updated_kickoffs.extend(std::mem::take(&mut ctx.new_kickoff_machines));
            ...
            if iterations > 100000 {
                return Err(eyre::eyre!(
                    r#"{}/{} kickoff and {}/{} round state machines did not stabilize after 100000 iterations, debug repr of changed machines:
                        ...
                        {:?}
                        "#,
                    updated_kickoffs.len(),
                    final_kickoff_machines.len() + updated_kickoffs.len(),
                    updated_rounds.len(),
                    final_round_machines.len() + updated_rounds.len(),
                    updated_kickoffs
                        .iter()
                        .map(|m| m.state())
                        .collect::<Vec<_>>(),
            ...
            let (finalized_kickoff_machines, new_kickoff_futures) =
                Self::match_blocks(&mut updated_kickoffs, ctx.cache.clone());
            let (finalized_round_machines, new_round_futures) =
                Self::match_blocks(&mut updated_rounds, ctx.cache.clone());
