...
> {
    ...
    let transaction = rpc.client.get_raw_transaction(&deposit_txid, None).await.expect("a");
    let tx_info: bitcoincore_rpc::json::GetRawTransactionResult = rpc.client.get_raw_transaction_info(&deposit_txid, None).await.expect("a");
    let block: bitcoincore_rpc::json::GetBlockResult = rpc.client.get_block_info(&tx_info.blockhash.unwrap()).await.expect("a");
    let block_height = block.height;
    let block = rpc.client.get_block(&tx_info.blockhash.unwrap()).await.expect("a");
    let transaction_params = get_transaction_params(transaction.clone(), block, block_height as u32, deposit_txid);
    println!("Deposit tx Transaction params: {:?}", transaction_params);
    println!("Deposit tx: {:?}", hex::encode(bitcoin::consensus::serialize(&transaction)));
    ...
    let transaction = rpc.client.get_raw_transaction(&move_txid, None).await.expect("a");
    let tx_info: bitcoincore_rpc::json::GetRawTransactionResult = rpc.client.get_raw_transaction_info(&move_txid, None).await.expect("a");
    let block: bitcoincore_rpc::json::GetBlockResult = rpc.client.get_block_info(&tx_info.blockhash.unwrap()).await.expect("a");
    let block_height = block.height;
    let block = rpc.client.get_block(&tx_info.blockhash.unwrap()).await.expect("a");
    let transaction_params = get_transaction_params(transaction.clone(), block, block_height as u32, move_txid);
    println!("Move tx Transaction params: {:?}", transaction_params);
    println!("Move tx: {:?}", hex::encode(bitcoin::consensus::serialize(&transaction)));
