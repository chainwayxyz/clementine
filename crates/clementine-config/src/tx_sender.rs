...
pub struct TxSenderLimits {
    ...
    /// Minimum fee bump increment in sat/kwu. If current fee rate is smaller than previously sent fee rate + min_bump_kwu, we do not bump at all. This is so that we do not do tiny fee bumps constantly.
    pub min_bump_kwu: u64,
}
