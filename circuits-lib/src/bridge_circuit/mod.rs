...
pub fn total_work_and_watchtower_flags(
    ...
    for (idx, ((tx, utxos), (input_idx))) in watchtower_challenge_txs
        .iter()
        ...
        .enumerate()
    {
        ...
        let witness = watchtower_tx.input[*input_idx as usize].witness.to_vec();
...
mod tests {
    ...
    fn test_total_work_and_watchtower_flags() {
        let wt_raw_tx = "";
        ...
        let sighash_t = sighash_man_implementation(&wt_tx, &kickoff_tx, 0);

        let outputs = [kickoff_tx.output[wt_tx.input[0].previous_output.vout as usize].clone()];
        let prevouts = Prevouts::All(&outputs);

        let sighash_2 = sighash(&wt_tx, &prevouts, 0, TapSighashType::Default);
        println!("Sighash: {}", hex::encode(&sighash_t));
        println!("Sighash 2: {}", hex::encode(&sighash_2.unwrap()));
        let msg_hash_hex = hex::encode(&sighash_t);
        let pubkey_hex = "412c00124e48ab8b082a5fa3ee742eb763387ef67adb9f0d5405656ff12ffd50";
        let sig_hex = "657306346d184257411f1b125df9a21deb1698d2444e07195e91b894bd49079fcaf002961888417f77b24251f6d899f9972f844d5360e845b6ab69c522729855";

        let secp = Secp256k1::verification_only();

        let msg_hash = decode(msg_hash_hex).unwrap();
        let msg = Message::from_digest(msg_hash.try_into().unwrap());

        let pubkey_bytes = decode(pubkey_hex).unwrap();
        let pubkey = XOnlyPublicKey::from_slice(&pubkey_bytes).unwrap();
        ...
        let watchtower_challenge_txs = vec![vec![0u8; 32]];
        let watchtower_challenge_utxos = vec![vec![0u8; 32]];
        let watchtower_challenge_witnesses = vec![vec![0u8; 32]];
        let num_watchtowers = 1;
        let work_only_image_id = [0u8; 32];

        // assert_eq!(total_work, [0u8; 32]);
        // assert_eq!(challenge_sending_watchtowers, [0u8; 20]);
    }
