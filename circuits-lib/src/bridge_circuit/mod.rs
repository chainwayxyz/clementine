...
use bitcoin::{
    ...
    opcodes,
    script::Instruction,
    sighash::{Annex, Prevouts, SighashCache, TaprootError},
    Script, TapSighash, TapSighashType, Transaction, TxOut, Witness,
};
...
fn sighash(
    ...
    let witness = wt_tx.input[input_index].witness.clone();
    let annex: Option<Annex> = if is_annex_present(&witness) {
        Some(Annex::new(witness.last().expect("Should not err")).expect("Should not err"))
    } else {
        None
    };
    SighashCache::new(wt_tx).taproot_signature_hash(
        input_index,
        prevouts,
        annex,
        None,
