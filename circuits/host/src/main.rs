...
use bridge_core::{btc::BlockHeader, config::N};
...
fn main() {
    ...
    env.write(&0).unwrap();           // start_deposit_index
    env.write(&0).unwrap();           // start_withdrawal_index
    env.write(&[7_u8; 32]).unwrap();  // initial_block_hash

    // env.write(&n).unwrap();
    env.write(&first_chain_work).unwrap();
    env.write(&[9_u8; 32]).unwrap();  // initial_bridge_funds_merkle_root
    ...
    let (mut start_deposit_index, start_withdrawal_index, initial_block_hash, initial_work, initial_bridge_funds_merkle_root, deposit_last_index, withdrawal_last_index, mut output_block_hash, light_client_pow, bridge_funds_merkle_root, last_finalized_block_hash, last_chain_work): (u32, u32, [u8; 32], [u8; 32], [u8; 32], u32, u32, [u8; 32], [u8; 32], [u8; 32], [u8; 32], [u8; 32]) =
